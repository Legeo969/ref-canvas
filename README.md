...
- 浏览器扩展（Chrome/Edge MV3）把网页图片直接发进参考板：本机 `127.0.0.1:17530` 捕获服务接收，捕获文件名唯一化防覆盖；每次导入自动归档进「网页捕获」集合，可一键认领复制到任意磁盘目录；无主孤儿文件启动时自动清理。
...
- 本地 Python/PowerShell 脚本：注册前确认路径、sha256 与将执行的命令行；运行前重新校验 hash（内容被改即拒跑）并校验工作目录范围，超时自动杀进程树。
...
所有索引与白板数据均保存在本机。分享服务、HTTP/QR、密码访问、云同步和账户服务不在当前版本范围内。
