...
describe("RefCanvasDatabase", () => {
  ...
  it("searches offset windows and batch-hydrates page relations", () => {
    ...
    const collection = database.createCollection("Window");
    database.setAssetTags(second.id, ["hydrated"]);
    database.addAssetToCollection(second.id, collection.id);
  ...
  it("persists tags and collection membership and reports global counts", () => {
    database = new RefCanvasDatabase(":memory:");
    const asset = database.upsertAsset(createAsset()).asset;
    const collection = database.createCollection("Architecture");
    database.setAssetTags(asset.id, ["mist", "temple", "mist"]);
    const updated = database.addAssetToCollection(asset.id, collection.id);
    ...
    expect(updated.collectionIds).toContain(collection.id);
    expect(database.searchAssets({ tag: "temple" }).items).toHaveLength(1);
    expect(
      database.searchAssets({ collectionId: collection.id }).items,
    ).toHaveLength(1);
  ...
  it("moves assets between collections without removing other memberships", () => {
    database = new RefCanvasDatabase(":memory:");
    const asset = database.upsertAsset(createAsset()).asset;
    const source = database.createCollection("Source");
    const target = database.createCollection("Target");
    const retained = database.createCollection("Retained");
    database.addAssetToCollection(asset.id, source.id);
    database.addAssetToCollection(asset.id, retained.id);

    database.batchUpdate(
      { mode: "ids", ids: [asset.id] },
      {
        addCollectionId: target.id,
        removeCollectionId: source.id,
      },
    );

    expect(database.getAsset(asset.id)?.collectionIds).toEqual(
      expect.arrayContaining([target.id, retained.id]),
    );
    expect(database.getAsset(asset.id)?.collectionIds).not.toContain(source.id);
  });
  ...
  it("supports nested folders and includes descendants in parent searches", () => {
    database = new RefCanvasDatabase(":memory:");
    const rootAsset = database.upsertAsset(createAsset()).asset;
    const childAsset = database.upsertAsset(
      createAsset({
        title: "Courtyard",
        path: "D:\\references\\courtyard.png",
        pathKey: "d:\\references\\courtyard.png",
        fingerprint: "courtyard",
      }),
    ).asset;
    const root = database.createCollection("Project");
    const child = database.createCollection("Architecture", root.id);
    database.addAssetToCollection(rootAsset.id, root.id);
    database.addAssetToCollection(childAsset.id, child.id);

    expect(
      database.searchAssets({ collectionId: root.id }).items.map((item) => item.id),
    ).toEqual(expect.arrayContaining([rootAsset.id, childAsset.id]));
    expect(database.getCollection(root.id)).toMatchObject({
      parentId: null,
      directAssetCount: 1,
      assetCount: 2,
    });
    expect(database.getCollection(child.id)).toMatchObject({
      parentId: root.id,
      directAssetCount: 1,
      assetCount: 1,
    });
    expect(() =>
      database!.updateCollection(root.id, { parentId: child.id }),
    ).toThrow("COLLECTION_CYCLE");
  });
