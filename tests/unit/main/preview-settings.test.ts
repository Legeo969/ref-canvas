...
describe("mergePreviewSettings（阶段 5：预览偏好持久化）", () => {
  ...
  it("规范化格式分组和非视觉白名单", () => {
    const next = mergePreviewSettings(PREVIEW_SETTINGS_DEFAULTS, {
      ...
      formatWhitelist: [".TXT", "txt", "md", "bad value"],
    });
    expect(next.formatGroups.find((group) => group.id === "image")?.extensions)
      .toEqual(["png", "exr"]);
    expect(next.formatGroups.find((group) => group.id === "video")?.extensions.length)
      .toBeGreaterThan(0);
    expect(next.formatWhitelist).toEqual(["txt", "md"]);
  });
});
