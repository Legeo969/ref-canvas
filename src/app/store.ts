...
interface AppState {
  ...
  /** 目录历史游标（directoryHistory 中的当前位置；0 = 最近）。 */
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  reloadAssets: async () => {
    ...
    const [page, stats] = await Promise.all([
      window.refCanvas.library.search(search),
  ...
  loadMore: async () => {
    ...
    const revision = assetQueryRevision;
    const search = get().currentSearch(nextCursor);
    set({ loadingMore: true });
    try {
      const page = await window.refCanvas.library.search(search);
      if (revision !== assetQueryRevision) return;
      set((state) => ({
        assets: [...state.assets, ...page.items],
        totalAssets: page.total,
        nextCursor: page.nextCursor,
      }));
    } finally {
