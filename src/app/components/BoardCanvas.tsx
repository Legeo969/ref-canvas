...
import { arrangeItems } from "../board-arrange";
import {
  DEFAULT_GIF_STATE,
  GifAnimator,
  gifStateFromData,
...
import {
  ...
  resolveBoardContext,
} from "../board-context-menu";
import { toolbarPanelPosition } from "../board-toolbar-position";
import { applyBoardControls } from "../board-controls";
import {
...
type Tool = "select" | "pencil" | "line" | "rectangle" | "ellipse";
type DrawingTool = Exclude<Tool, "select">;
...
function isShapeDrawingTool(tool: Tool): tool is Exclude<DrawingTool, "pencil"> {
...
export function BoardCanvas({
  ...
  const drawingStyleRef = useRef<DrawingStyle>(defaultDrawingStyle);
  ...
  const [drawingStyle, setDrawingStyle] =
    useState<DrawingStyle>(defaultDrawingStyle);
  ...
  const scenePointFromClient = (clientX: number, clientY: number) => {
    ...
    return {
      x: (clientX - left - vpt[4]) / vpt[0],
      y: (clientY - top - vpt[5]) / vpt[3],
    };
  };
  ...
  useEffect(() => {
    ...
    canvas.on("contextmenu", ({ e, target: foundTarget }) => {
      ...
      const target = foundTarget as CanvasObjectWithData | undefined;
    ...
    const scheduleSave = () => {
      ...
      historyFrame = window.requestAnimationFrame(() => {
        ...
        if (historyRef.current[historyIndexRef.current] !== snapshot) {
          const limit = Math.max(1, undoLimitRef.current);
          historyRef.current = historyRef.current.slice(
            Math.max(0, historyIndexRef.current - (limit - 1)),
    ...
    canvas.on("selection:created", (event) => {
      ...
      if (asset || !target?.data?.assetId) {
        onSelectAssetRef.current(asset ?? null);
      } else {
        void window.refCanvas.library
          .get(target.data.assetId)
    ...
    let drawingObject: CanvasObjectWithData | null = null;
    ...
    canvas.on("mouse:down", (event) => {
      ...
      if (isShapeDrawingTool(toolRef.current)) {
        const point = canvas.getScenePoint(pointerEvent);
        const style = drawingStyleRef.current;
        const shared = {
          stroke: style.color,
    ...
    canvas.on("mouse:move", (event) => {
      ...
      if (drawingObject instanceof Line) {
        ...
        drawingObject.set({ x2: end.x, y2: end.y });
      } else {
        ...
        if (drawingObject instanceof Rect) {
          drawingObject.set(bounds);
        } else if (drawingObject instanceof Ellipse) {
          drawingObject.set({
            left: bounds.left,
            top: bounds.top,
            rx: bounds.width / 2,
  ...
  useEffect(() => {
    ...
    canvas.selection = tool === "select" && !canvasModeRef.current.locked;
    canvas.skipTargetFind = tool !== "select" || canvasModeRef.current.locked;
  ...
  const toggleEraser = () => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const next = !eraserActiveRef.current;
    eraserActiveRef.current = next;
    setEraserActive(next);
    canvas.selection = !next;
    canvas.skipTargetFind = next;
    canvas.defaultCursor = next ? "not-allowed" : "default";
  ...
  const eraseAtPoint = (point: { x: number; y: number }) => {
    const canvas = canvasRef.current;
    if (!canvas || !eraserActiveRef.current) return;
    const pointInScene = new Point(
      (point.x - (canvas.viewportTransform?.[4] ?? 0)) / (canvas.getZoom() || 1),
      (point.y - (canvas.viewportTransform?.[5] ?? 0)) / (canvas.getZoom() || 1),
    );
    const hit = canvas
      .getObjects()
      .find(
        (object) =>
          (object as CanvasObjectWithData).data?.type === "drawing-pencil" &&
          object.containsPoint(pointInScene),
      );
    if (hit) {
      canvas.remove(hit);
      strokeHistoryRef.current = strokeHistoryRef.current.filter(
  ...
  const fitObjects = (selectionOnly = false) => {
    ...
    const zoom = Math.min(
      4,
      Math.max(0.08, Math.min((canvas.width - 96) / width, (canvas.height - 96) / height)),
    );
  ...
  const toggleCanvasLock = () => {
    ...
    setSaved(false);
    void onSaveRef.current(makeDocument(canvas));
  };
  ...
  useEffect(() => {
    const applySettings = (settings: BoardSettings) => {
      undoLimitRef.current = settings.undoLimit;
        ...
        const batch = await Promise.all(
          droppedAssets.slice(index, index + 8).map((asset, offset) => {
            ...
            return addAsset(
              asset,
              {
                x: position.x + positions[absoluteIndex].x,
                y: position.y + positions[absoluteIndex].y,
              },
              { activate: false },
            );
          }),
      ...
      >
        ...
        <div className="canvas-toolbar toolbar-compact" role="toolbar" aria-label="白板工具">
          ...
          <button
            className={`toolbar-primary ${tool !== "select" ? "active" : ""}`}
            onClick={() =>
              setTool(tool === "select" ? lastDrawingTool : "select")
            }
            aria-label={tool === "select" ? "启用绘图工具" : "退出绘图工具"}
          >
            <Paintbrush size={16} />
          </button>
          <button
          ...
          <button
            className={`toolbar-primary ${eraserActive ? "active" : ""}`}
          ...
          <button
            ...
            className={canvasModeRef.current.locked ? "active" : ""}
            aria-label={canvasModeRef.current.locked ? "解锁画布" : "锁定整个画布"}
            title={canvasModeRef.current.locked ? "解锁画布" : "锁定整个画布"}
          >
            <Lock size={16} />
          </button>
          <button
          ...
          >
            ...
            {boardContextMenu.kind === "empty" && (
              <>
                ...
                <div className="folder-menu-separator" />
                <button role="menuitem" onClick={() => setCommandPaletteOpen(true)}>
                  <Search size={16} />
                  命令面板
                </button>
              </>
