...
interface DirectoryQuickPreviewProps {
  ...
  onMaterialize(entry: DirectoryEntry): void;
  onAddToCollection(entry: DirectoryEntry): void;
...
export function DirectoryQuickPreview({
  ...
  const previewContent = () => {
    ...
    if (kind === "image") {
      return (
        <img
          ...
          className={`progressive-preview ${fullImageReady ? "ready" : "proxy"}`}
          alt=""
          draggable={false}
    ...
    if (kind === "video") {
      return (
        <video src={rawUrl!} controls preload="metadata">
          <track kind="captions" />
        </video>
      );
    }
