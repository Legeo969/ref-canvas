...
import { ancestorChain, folderLabel } from "../app/folder-navigation";
...
const gap = 12;
const maxPhysicalScrollHeight = 16_000_000;
...
export function AssetPanel() {
  ...
  const rowCount = Math.ceil(store.totalAssets / columns);
  const logicalScrollHeight = Math.max(itemRowHeight, rowCount * itemRowHeight);
  const physicalScrollHeight = Math.min(
    maxPhysicalScrollHeight,
    logicalScrollHeight,
  );
  const scrollScale = logicalScrollHeight / physicalScrollHeight;
  const logicalViewportTop = viewport.top * scrollScale;
  const startRow = Math.max(
    0,
    Math.floor(logicalViewportTop / itemRowHeight) - 2,
  );
  const endRow = Math.min(
    rowCount,
    Math.ceil(
      (logicalViewportTop + viewport.height * scrollScale) / itemRowHeight,
    ) + 3,
  );
  const visibleAssets = useMemo(
    () => {
      const visibleStart = startRow * columns;
      const visibleEnd = Math.min(store.totalAssets, endRow * columns);
      const loadedStart = Math.max(visibleStart, store.assetWindowOffset);
      const loadedEnd = Math.min(
        visibleEnd,
        store.assetWindowOffset + store.assets.length,
      );
      if (loadedStart >= loadedEnd) return [];
      return store.assets
        .slice(
          loadedStart - store.assetWindowOffset,
          loadedEnd - store.assetWindowOffset,
        )
        .map((asset, index) => ({ asset, absoluteIndex: loadedStart + index }));
    }, [
      store.assets,
      store.assetWindowOffset,
      store.totalAssets,
      startRow,
      endRow,
      columns,
  ...
  const firstVisibleRow = Math.floor(logicalViewportTop / itemRowHeight);
  const lastVisibleRow = Math.ceil(
    (logicalViewportTop + viewport.height * scrollScale) / itemRowHeight,
  );

  useEffect(() => {
    if (!store.totalAssets) return;
    void store.ensureAssetRange(
      startRow * columns,
      Math.max(startRow * columns, endRow * columns - 1),
    );
  }, [columns, endRow, startRow, store.ensureAssetRange, store.totalAssets]);
  ...
  const focusAssetCard = (assetId: string, index: number) => {
    ...
    if (node) {
      const rowTop =
        (Math.floor(index / columns) * itemRowHeight) / scrollScale;
      const physicalRowHeight = itemRowHeight / scrollScale;
      if (rowTop < node.scrollTop) {
        node.scrollTop = rowTop;
      } else if (
        rowTop + physicalRowHeight >
        node.scrollTop + node.clientHeight
      ) {
        node.scrollTop = rowTop + physicalRowHeight - node.clientHeight;
      }
    }
  ...
  const navigateGrid = async (
    ...
    const localIndex = state.assets.findIndex(
      (asset) => asset.id === state.selectedAsset?.id,
    );
    const currentIndex =
      localIndex < 0 ? -1 : state.assetWindowOffset + localIndex;
    const target = assetGridNavigationTarget({
      key,
      currentIndex,
      itemCount: state.totalAssets,
      columns,
      visibleRows: Math.max(1, Math.floor(viewport.height / itemRowHeight)),
      canLoadMore: false,
    });
    if (!target) return;
    await state.ensureAssetRange(target.index, target.index);
    state = useAppStore.getState();
    const asset = state.assetAt(target.index);
    if (!asset) return;
  ...
  return (
    ...
    >
        ...
        >
          {detailMode ? (
            <div
              ...
              style={{ height: physicalScrollHeight }}
            >
              {visibleAssets.map(({ asset, absoluteIndex }) => {
                const selected =
                ...
                return (
                  <button
                    ...
                    style={{
                      top:
                        viewport.top +
                        (absoluteIndex * detailRowHeight - logicalViewportTop),
                    }}
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    top:
                      viewport.top +
                      (row * cardRowHeight - logicalViewportTop),
                    width: cardWidth,
                  }}
      ...
      {previewId && store.assets.some((asset) => asset.id === previewId) && (
        <QuickPreview
          assets={store.assets}
          windowOffset={store.assetWindowOffset}
          ...
          onNavigateIndex={async (index) => {
            await store.ensureAssetRange(index, index);
            const next = useAppStore.getState().assetAt(index);
            if (!next) return;
            setPreviewId(next.id);
            useAppStore.getState().selectAssetInGrid(next.id, "replace");
          }}
