  ...
  return (
    <div className="settings-group about-group">
      ...
      <dl className="about-list">
        <div><dt>{translate("settings.about.installChannel")}</dt><dd>{appInfo?.installChannel ?? "…"}</dd></div>
        <div><dt>Electron</dt><dd>{appInfo?.electronVersion ?? "…"}</dd></div>
        <div><dt>Node</dt><dd>{appInfo?.nodeVersion ?? "…"}</dd></div>
        <div><dt>{translate("settings.about.databaseSchema")}</dt><dd>{appInfo?.databaseSchemaVersion ?? "…"}</dd></div>
        <div>
          <dt>{translate("settings.about.indexDatabase")}</dt>
          <dd title={appInfo?.libraryPath ?? undefined}>{appInfo?.libraryPath ?? translate("settings.about.libraryNotOpen")}</dd>
        </div>
        <div><dt>{translate("settings.about.platform")}</dt><dd>{appInfo?.platform ?? "…"}</dd></div>
      </dl>
      <div className="about-actions">
        <button className="secondary-button" onClick={() => void window.refCanvas.system.writeClipboard(versionInfo)}>
          <Clipboard size={15} />{translate("settings.about.copyVersion")}
        </button>
        <button className="secondary-button" onClick={() => void window.refCanvas.system.openDataFolder()}>
          <FolderOpen size={15} />{translate("settings.about.openDataFolder")}
        </button>
        <button
