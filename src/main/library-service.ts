...
import type { NewAsset, RefCanvasDatabase } from "./database";
...
export class LibraryService {
  ...
  private emitJob(job: { snapshot: ImportJobSnapshot }): void {
    this.events.emit("import-progress", structuredClone(job.snapshot));
  }
  ...
  private async runImport(
    ...
    if (parentFolderId && !this.database.getCollection(parentFolderId)) {
      ...
      // startImport 在订阅前同步调用本方法；延迟到下一轮事件循环再发终态，
      // 否则 importPaths 会错过事件而永远等待。
      setImmediate(() => this.emitJob(job));
      return;
    }
    try {
      ...
      for (const inputPath of snapshot.sourcePaths) {
        if (controller.signal.aborted) break;
        const resolved = path.resolve(inputPath);
        const fileStat = await stat(resolved);
        if (fileStat.isDirectory()) {
          const watchRoot = watchRoots.find(
            (root) => resolved === root || resolved.startsWith(`${root}${path.sep}`),
          );
          files.push(
            ...(await walk(resolved, controller.signal)).map((filename) => ({
              filename,
              collectionSegments: segmentsForFile(filename, watchRoot ?? resolved),
              ...(watchRoot ? { watchRootPath: watchRoot } : {}),
            })),
          );
          ...
          files.push({
            filename: resolved,
            collectionSegments: watchRoot
              ? segmentsForFile(resolved, watchRoot)
              : [],
            ...(watchRoot ? { watchRootPath: watchRoot } : {}),
          });
        }
      }
      snapshot.discovered = files.length;
      // Every discovered file is a candidate: readable files with unknown
      // extensions are imported as `generic`, unreadable ones are reported.
      const supported = files;
      snapshot.state = "processing";
      this.emitJob(job);
      for (let index = 0; index < supported.length; index += 12) {
        if (controller.signal.aborted) break;
        const candidates = supported.slice(index, index + 12);
        const inspected = await Promise.all(
          candidates.map(async (candidate) => {
            try {
              const existing = this.database.getAssetByPath(candidate.filename) ?? undefined;
              const linked = await this.readAsset(candidate.filename, existing);
              ...
              return { candidate, asset: managed, copied: true, verified: true };
            } catch (error) {
              snapshot.failed.push({
        ...
        snapshot.verified += valid.filter((item) => item.verified).length;
        const savedAssets = this.database.upsertAssets(
          valid.map((item) => item.asset),
        );
        const collectionRelations: Array<{
          assetId: string;
          collectionId: string;
        }> = [];
        ...
        for (let savedIndex = 0; savedIndex < savedAssets.length; savedIndex += 1) {
          const saved = savedAssets[savedIndex];
          const item = valid[savedIndex];
          ...
          if (storageMode === "linked") {
            const rootPath = watchRoots.find(
              (root) =>
                saved.asset.path === root ||
                saved.asset.path.startsWith(`${root}${path.sep}`),
            );
            if (rootPath) {
              identities.push({
                pathKey: path
                  .normalize(saved.asset.path)
          ...
          const segments = item.candidate.collectionSegments;
          // parentFolderId 时树整体嵌套在该文件夹下（根段直接成为其子文件夹）。
          ...
          for (const segment of segments) {
            ...
            if (!collectionId) {
              collectionId = this.database.findOrCreateCollectionId(
                segment,
                parentId,
              );
        ...
        if (collectionRelations.length) {
          this.database.addAssetsToCollections(collectionRelations);
        }
        snapshot.processed = Math.min(index + candidates.length, supported.length);
        this.emitJob(job);
      }
  ...
  retryImport(id: string): ImportJobSnapshot {
    ...
    return this.startImport(
      failedPaths.length ? failedPaths : job.snapshot.sourcePaths,
      job.options,
    );
  }
  ...
  async addWatchRoot(root: string): Promise<ImportResult> {
    ...
    if (this.watcher) this.watcher.add(resolved);
    else if (this.watching && process.platform === "win32") {
      this.startNativeWatchRoot(resolved, watchRoot.id);
    }
  ...
  async removeWatchRoot(id: string): Promise<WatchRoot> {
    ...
    if (this.watcher) await this.watcher.unwatch(root.path);
    const nativeWatcher = this.nativeWatchers.get(path.resolve(root.path));
    nativeWatcher?.close();
    this.nativeWatchers.delete(path.resolve(root.path));
  ...
  private startNativeWatchRoot(root: string, rootId?: string): void {
    const resolvedRoot = path.resolve(root);
    if (this.nativeWatchers.has(resolvedRoot)) return;
    const watcher = watchNative(
      resolvedRoot,
      { recursive: true },
      (_eventType, relativeFilename) => {
        if (!relativeFilename) {
        ...
        const filename = path.resolve(
          resolvedRoot,
          relativeFilename.toString(),
        );
        if (this.managedPaths.has(path.normalize(filename))) return;
        const existingTimer = this.nativeWatchTimers.get(filename);
  ...
  async startWatching(): Promise<void> {
    ...
    if (!roots.length || this.watching) return;
    this.watching = true;
    ...
    if (process.platform === "win32") {
      for (const root of roots) this.startNativeWatchRoot(root.path, root.id);
      return;
    }
    ...
    this.watcher.on("change", (filename) => {
      if (!this.managedPaths.has(path.normalize(filename))) void this.importPaths([filename]);
    });
      ...
      if (!this.managedPaths.has(path.normalize(filename))) {
        ...
        if (asset) {
          ...
          const watchRoot = this.database.listWatchRoots().find(
            (root) => filename === root.path || filename.startsWith(`${root.path}${path.sep}`),
          );
          if (watchRoot) this.database.pruneEmptyGeneratedCollections(watchRoot.path);
          this.emitLibraryChanged({ reason: "watch", paths: [filename] });
        }
      }
    });
